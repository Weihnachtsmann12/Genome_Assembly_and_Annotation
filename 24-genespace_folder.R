...
# Load the annotation
annotation <- fread("/data/users/mrubin/assembly_annotation_course/gene_annotation_directory/assembly.maker.output/final/filtered.genes.renamed.final.gff3", header = FALSE, sep = "\t")
...
top20_scaff <- fread("/data/users/mrubin/assembly_annotation_course/flye_output_2.9.5/Flye.fai", header = FALSE, sep = "\t") %>%
    select(V1, V2) %>%
    arrange(desc(V2)) %>%
    head(20)
...
longest_proteins <- "/data/users/mrubin/assembly_annotation_course/gene_annotation_directory/assembly.maker.output/final/assembly.all.maker.proteins.fasta.renamed.longest.fasta"
