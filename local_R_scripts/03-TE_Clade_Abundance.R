### 3. Refining TE Classification with TEsorter for Flye assembly
## Make plots to show TE Clade Abundance

library(dbplyr)
library(ggplot2)
library(tidyr)

gypsy_name <- "Gypsy_sequences_flye.fa.rexdb-plant.cls.tsv"
gypsy <- read.csv(gypsy_name, sep="\t")
gypsy <- gypsy %>%
  separate(X.TE, into = c("V1", "_"), sep = "#")

copia_name <- "Copia_sequences_flye.fa.rexdb-plant.cls.tsv"
